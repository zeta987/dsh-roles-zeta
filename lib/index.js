...
export function apply(ctx, config) {
  ...
  const { roles, aliases, warnings, origin } = loadRoles([
    { dir: PACKAGED_ROLES_DIR, source: 'packaged', required: true },
    { dir: settings.rolesDir, source: 'user' },
  ])
  ...
  const fromUser = [...origin.entries()].filter(([, source]) => source === 'user').length
  ctx.logger?.info?.(
    `roles-zeta: ${roles.size} roles loaded, ${fromUser} read from "${settings.rolesDir}"`,
  )
