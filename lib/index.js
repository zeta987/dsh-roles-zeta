...
export function apply(ctx, config) {
  ...
  // Say where the roster came from once, so a user who expects the shipped roles
  // to appear in their own directory is not left guessing.
  const fromUser = [...origin.entries()].filter(([, source]) => source === 'user').map(([id]) => id)
  ctx.logger?.info?.(
    `roles-zeta: ${roles.size} roles loaded (${fromUser.length} from "${settings.rolesDir}"` +
      `${fromUser.length === 0 ? '' : `: ${fromUser.join(', ')}`}); the rest ship with the package`,
  )
