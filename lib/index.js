/**
 * The dsh-role-agents plugin entry.
 *
...
function assertProvider(provider, settings, needsToolFilter) {
  const label = `role-agents: provider "${provider.name}"`
...
export function apply(ctx, config) {
  ...
  for (const warning of warnings) ctx.logger?.warn?.(`role-agents: ${warning}`)

  if (roles.size === 0) {
    ctx.logger?.warn?.(
      `role-agents: no roles loaded from "${settings.rolesDir}"; "${settings.toolName}" is not registered`,
    )
    return
  }
  ...
  const mount = (provider) => {
    ...
    ctx.logger?.info?.(
      `role-agents: registered "${settings.toolName}" over provider "${provider.name}" ` +
  ...
  if (present !== undefined) mount(present)
    ...
    ctx.logger?.info?.(
      `role-agents: subagent provider "${settings.provider}" is not registered yet; ` +
