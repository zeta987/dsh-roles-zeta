 ...
 *
 * Around the tool sit three optional surfaces, each mounted only where its host
 * service exists: one slash command per role (`ctx.commands`), a settings page
 * API over the role directory (`ctx.webServer`), and a directory watcher that
 * reloads the roster when a file changes. All of them read the same live
 * library, so an edit reaches the tool schema, the `/` menu, and the page
 * together.
 *
...
import { createRoleLibrary } from './library.js'
import { seedRoles } from './seed.js'
import { installRoleCommands } from './commands.js'
import { installRolesApi } from './api.js'
import {
  resolveRouteAlias,
  resolveInlineModel,
  matchRoute,
...
/**
 * The roster this package ships, read from inside the installed package.
 *
 * `lib/index.js` sits one level below the package root, so the shipped roles are
 * its sibling. They load first and every one of them can be replaced or removed
 * by a file in the user's own directory.
 */
const PACKAGED_ROLES_DIR = fileURLToPath(new URL('../roles/', import.meta.url))
...
function normalizeConfig(config) {
  ...
  return {
    ...
    commandPrefix: String(merged.commandPrefix ?? ''),
  }
}
 ...
 * @param agent - the calling agent, which is the viewing scope; omitted for the global view.
 * @returns the visible names, or undefined when the registry could not answer.
 */
function knownToolNames(ctx, agent) {
  try {
    const schemas = agent === undefined ? ctx.tools.schemas() : ctx.tools.schemas(agent)
...
/** Register a teardown on the plugin context when it offers effects. */
function onDispose(ctx, dispose, label) {
  if (typeof ctx.effect === 'function') ctx.effect(() => dispose, label)
}
...
export function apply(ctx, config) {
  ...
  // Put the shipped roster on disk before reading anything, so the files are
  // where a dsh user looks for agent definitions, and still follow the package
  // while they are untouched.
  if (settings.seedRolesDir !== false) {
    const seeded = seedRoles({
      packagedDir: PACKAGED_ROLES_DIR,
      rolesDir: settings.rolesDir,
      logger: ctx.logger,
    })
    if (seeded.error !== undefined) {
      ctx.logger?.warn?.(`roles-zeta: not seeding roles: ${seeded.error}`)
    } else {
      const placed = seeded.seeded.length
      const refreshed = seeded.refreshed.length
      if (placed > 0 || refreshed > 0) {
        ctx.logger?.info?.(
          `roles-zeta: ${placed} role file(s) placed in "${settings.rolesDir}"` +
            (refreshed === 0 ? '' : `, ${refreshed} refreshed from the package`) +
            (seeded.kept.length === 0 ? '' : `, ${seeded.kept.length} left as yours`),
        )
      }
      for (const name of seeded.missing) {
        ctx.logger?.info?.(`roles-zeta: "${name}" was removed from "${settings.rolesDir}"; not restoring it`)
      }
    }
  }

  const library = createRoleLibrary({
    packagedDir: PACKAGED_ROLES_DIR,
    rolesDir: settings.rolesDir,
    logger: ctx.logger,
  })
  ...
  const describeRoster = () => {
    const fromUser = [...library.origin.entries()].filter(([, source]) => source === 'user').length
    return `${library.roles.size} roles loaded, ${fromUser} read from "${settings.rolesDir}"`
  }
  ctx.logger?.info?.(`roles-zeta: ${describeRoster()}`)
  ...
  const prepare = async (args, exec) => {
    const role = library.find(args.role)
    if (role === undefined) {
      throw new Error(`unknown role "${args.role}"; available roles: ${[...library.roles.keys()].join(', ')}`)
    }
  ...
  /** The tool for the roster as it is right now; rebuilt whenever the roster changes. */
  const buildTool = () => {
    const roleIds = [...library.roles.keys()]
    const allowedForDescription = allowedRoutes(selectionService)
    return defineTool({
      name: settings.toolName,
      description: buildDescription(library.roles, routeAliases, allowedForDescription),
      parameters: {
        role: {
          type: 'string',
          required: true,
  ...
  // ── the tool, mounted over the provider and rebuilt when the roster changes ──
  ...
  const unregisterTool = () => {
    ...
    try {
      ...
      // The registry may already have dropped it.
    }
  ...
  const registerTool = () => {
    ...
    if (library.roles.size === 0) {
      unregisterTool()
      ctx.logger?.warn?.(
        `roles-zeta: no roles loaded from "${PACKAGED_ROLES_DIR}" or "${settings.rolesDir}"; ` +
          `"${settings.toolName}" is not registered`,
      )
      return
    }
    // Validate and build before touching the current registration, so a
    // roster the provider cannot serve leaves the previous tool in place
    // instead of leaving no tool at all.
    const needsToolFilter = [...library.roles.values()].some(
      (role) => role.allow !== undefined || role.delegation !== true,
    )
    assertProvider(provider, settings, needsToolFilter)
    const tool = buildTool()
    unregisterTool()
    registration = ctx.tools.register(tool)
    ctx.logger?.info?.(
      `roles-zeta: registered "${settings.toolName}" over provider "${provider.name}" ` +
        `with ${library.roles.size} roles`,
    )
  }

  ctx.on('subagent/provider-added', (added) => {
    if (added.name === settings.provider && provider === undefined) {
      provider = added
      registerTool()
    }
  })
  ctx.on('subagent/provider-removed', (providerName) => {
    if (providerName !== settings.provider || provider === undefined) return
    unregisterTool()
    provider = undefined
  })
  ...
  const unsubscribe = library.onChange(() => {
    ctx.logger?.info?.(`roles-zeta: roster changed; ${describeRoster()}`)
    registerTool()
  })
  onDispose(
    ctx,
    ...
    'roles-zeta: delegate tool',
  )

  // ── the optional surfaces ──

  if (settings.watchRolesDir !== false) {
    onDispose(ctx, library.watch(), 'roles-zeta: role directory watcher')
  }

  if (settings.commands !== false && typeof ctx.inject === 'function') {
    ctx.inject(['commands'], (commandCtx) => {
      const dispose = installRoleCommands(commandCtx, library, {
        toolName: settings.toolName,
        prefix: settings.commandPrefix,
        logger: ctx.logger,
      })
      onDispose(commandCtx, dispose, 'roles-zeta: slash commands')
    })
  }

  if (settings.settingsPage !== false && typeof ctx.inject === 'function') {
    ctx.inject(['webServer', 'connection'], (webCtx) => {
      const dispose = installRolesApi(webCtx, library, {
        packagedDir: PACKAGED_ROLES_DIR,
        settings,
        selectionService,
        getKnownTools: () => knownToolNames(ctx) ?? [],
        getRegisteredRoutes: registeredRoutes,
        logger: ctx.logger,
      })
      onDispose(webCtx, dispose, 'roles-zeta: settings API')
    })
  }
}
